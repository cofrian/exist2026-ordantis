"""EXPERIMENTO A: Task 2.2 con XLM-R-base a max_length=512 (el máximo nativo) y SIN
el campo `reasoning` del texto enriquecido. NO toca la submission actual: guarda
checkpoint en _alt/ y predicciones en _alt/runs/. Reentrena Vista E-2.2 y compara
métricas en validación con el modelo actual (max_length=320 con reasoning)."""
...
ALT = os.path.join(C.OUT_DIR, "_alt"); os.makedirs(os.path.join(ALT, "runs"), exist_ok=True)
INT = ["NO", "DIRECT", "JUDGEMENTAL"]; HIER = {"YES": ["DIRECT", "JUDGEMENTAL"], "NO": []}
TC = "EXIST2025"; EPS = 1e-7; SEED = 999
MAX_TOK = 512   # ← clave
P1, P2, PAT = 3, 10, 4
DEV = C.DEVICE
CKPT = os.path.join(ALT, "vista_e_task22_max512_best.pt")
...
def load_t22():
    ...
    def enrich(e):
        ...
        # ⚠️ SIN reasoning (cambio clave del experimento A)
        txt = f"{ocr} </s> {desc} </s> INTENTION: {ir} </s> IRONY: {irony_s} </s> {sa}"
...
def warm_start(model):
    ...
    print(f"  [warm-start] XLM-R {len(tm)} tensores + EEG pool {len(ep)}", flush=True)
...
def main():
    ...
    print(f"[EXP-A max512] cargando datos ... MAX_TOK={MAX_TOK} sin reasoning", flush=True)
    splits = load_t22()
    tok = AutoTokenizer.from_pretrained(C.TEXT_MODEL); cl = collate(tok)
    dl_tr = DataLoader(DS22(splits["train"]), batch_size=8, sampler=sampler_for(splits["train"]),  # bs reducido por L=512
                       collate_fn=cl, num_workers=4, pin_memory=True)
    dl_va = DataLoader(DS22(splits["val"]), batch_size=32, shuffle=False, collate_fn=cl, num_workers=4)
    model = VistaE22().to(DEV); warm_start(model)
    ...
    # ---- evaluación final + thresholds ponderados + Platt
    ...
    for w,tj,td in cands[:8]:
        ...
        if best is None or sc>best[0]: best=(sc,tj,td)
    _, tjE, tdE = best
    ...
    print(f"\n=== RESULTADOS Task 2.2 EXP-A (max_length={MAX_TOK}, sin reasoning) ===", flush=True)
    ...
    # Comparación con el modelo actual (que está en CKPT_DIR/vista_e_task22_best.pt)
    print("\n  comparación con el modelo ACTUAL (max_length=320 + reasoning):", flush=True)
    print(f"   actual:  HARD ICM thr+0.035  F1macro 0.560 | SOFT (Vista E+Platt) ICMSoft -0.439", flush=True)
    print(f"   nuevo:   HARD ICM thr {icm_thr:+.4f}  F1macro {f1_score(y,dec(vP,tjE,tdE),average='macro',labels=[0,1,2]):.4f} | SOFT (Platt) ICMSoft {icms_platt:+.4f}", flush=True)
