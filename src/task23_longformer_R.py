...
MAX_TOK = 1100
P1, P2, PAT = 2, 8, 3
CKPT = os.path.join(ALT, "vista_e_task23_longformer_R_best.pt")

# reutilizar load_t23, DS23, gold_hard, pred_hard, pyevall_* del max512
from task23_max512_R import load_t23, DS23, cat_posw, sampler_for, loss_fn, gold_hard, pred_hard, pyevall_hard, pyevall_soft
...
def main():
    ...
    print(f"[EXP-B 2.3 Longformer+SAMPLER+REASONING] {LONG_MODEL} MAX_TOK={MAX_TOK}", flush=True)
    splits = load_t23()
    tok = AutoTokenizer.from_pretrained(LONG_MODEL); cl = collate(tok)
    dl_tr = DataLoader(DS23(splits["train"]), batch_size=4, sampler=sampler_for(splits["train"]), collate_fn=cl, num_workers=4, pin_memory=True)
