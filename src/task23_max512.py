...
MAX_TOK = 512   # ← clave
P1, P2, PAT = 3, 12, 4
CKPT = os.path.join(ALT, "vista_e_task23_max512_best.pt")
...
def load_t23():
    ...
    def enrich(e):
        ...
        txt = f"{ocr} </s> {desc} </s> CATEGORIES: {present} </s> {cr} </s> {sa}"
...
def main():
    ...
    print(f"[EXP-A 2.3 max512] MAX_TOK={MAX_TOK}", flush=True)
    splits = load_t23()
    tok = AutoTokenizer.from_pretrained(C.TEXT_MODEL); cl = collate(tok)
    dl_tr = DataLoader(DS23(splits["train"]), batch_size=8, shuffle=True, collate_fn=cl, num_workers=4, pin_memory=True)
